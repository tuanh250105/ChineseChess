public class Xe extends Piece {
    ...
    public String getSymbol() {
        return color.equals("Trắng") ? "車" : "車";  // Xe Trắng hoặc Xe Đen (cùng biểu tượng)
    }
    @Override
    public boolean isValidMove(int newRow, int newCol, Piece[][] board) {
        // Xe di chuyển theo hàng hoặc cột, không thể vượt qua quân khác
        if (row != newRow && col != newCol) return false; // Không phải di chuyển theo hàng hoặc cột
        ...
        if (row == newRow) { // Di chuyển theo hàng
            ...
            for (int i = minCol; i < maxCol; i++) {
                if (board[row][i] != null) return false; // Có quân cản trở
            }
        } else if (col == newCol) { // Di chuyển theo cột
            int minRow = Math.min(row, newRow) + 1;
            int maxRow = Math.max(row, newRow);
            for (int i = minRow; i < maxRow; i++) {
                if (board[i][col] != null) return false; // Có quân cản trở
            }
        }
        return true; // Nếu không có vấn đề gì, nước đi hợp lệ
    }
}
